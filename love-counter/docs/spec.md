...
#### 1. **LIFF（LINE Front-end Framework）によるユーザー識別**
   - LINEアプリ内でWebアプリを開く
   - ユーザーIDを自動取得（誰がアプリを開いたか分かる）
   - そのユーザーのトークルームにメッセージを送信
   - 実装場所: `index.html`（LIFF SDK組み込み）

#### 2. **メッセージ送信機能**
   - アプリ内の「送信」ボタンからLINEにメッセージを送信
   - 送信先: アプリを開いたユーザーとのトークルーム
   - 実装場所: `api/send-message.js`

#### 3. **Webhook自動応答**
   - アプリからメッセージが届いたら自動返信
   - 応答例: 「メッセージありがとう！あとでちゃんと返信するね😊」
   - 実装場所: `api/webhook.js`

#### 4. **キーワード応答**
   - ユーザーが「カウンター」と送ったらLIFF URLを返信
   - LINE公式アカウントの管理画面で設定
   - これにより、LINE公式アカウントの通常の自動応答と重複しない

### 動作フロー

```
1. ユーザーがLINE公式アカウントに「カウンター」と送信
   ↓
2. LINE公式アカウントのキーワード応答が反応
   ↓
3. LIFF URLを返信（例: https://liff.line.me/xxxxx）
   ↓
4. ユーザーがURLをタップ → アプリがLINEアプリ内で起動
   ↓
5. LIFF SDKでユーザーIDを自動取得
   ↓
6. ユーザーがりんごボタンをタップして「送信」
   ↓
7. api/send-message.js がそのユーザーにメッセージを送信
   ↓
8. api/webhook.js が自動応答
   「メッセージありがとう！あとでちゃんと返信するね😊」
   ↓
9. 管理者（開発者）がLINE公式アカウント管理画面でメッセージを確認
   ↓
10. 手動で個別に返信
```

### 技術構成
- **API**: LINE Messaging API + LIFF（LINE Front-end Framework）
- **バックエンド**: Vercel Serverless Functions
- **認証**: 環境変数（`.env.local`）
- **フロントエンド**: LIFF SDK組み込み
