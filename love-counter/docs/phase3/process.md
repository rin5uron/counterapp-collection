...
#### エラー4: 「This channel is now developing status」エラー

**問題:**
- ログインボタンを押すと以下のエラーが表示される
- 「This channel is now developing status. User need to have developer role.」
- このチャネルは開発中ステータスです。開発者ロールを持つユーザーのみがアクセスできます

**原因:**
- **LINEログインチャネルが「開発中（Developing）」ステータスのまま**
- 開発中のチャネルは、開発者として登録されたユーザーしかログインできない
- 一般ユーザー（友達）はアクセスできない

**解決方法:**

1. **チャネルを公開ステータスに変更**
   - LINE Developers Console → Love Counter Login → チャネル基本設定
   - **「ステータス」** の項目を探す
   - **「Developing（開発中）」** → **「Published（公開）」** に変更
   - 保存
...
2. **公開に必要な情報を設定**
   ...
   **仮のURLで設定してもOK：**
   ```
   プライバシーポリシーURL: https://love-counter-theta.vercel.app/
   サービス利用規約URL: https://love-counter-theta.vercel.app/
   ```

**実施内容（2025-12-06）:**
- Love Counter Login チャネルのステータスを「Published（公開）」に変更
- これにより、友達追加した一般ユーザーもLIFFアプリにログインできるようになった
...
1. **LIFFのエンドポイントURLは完全一致が必要**
   ...
   - 実際のVercel デプロイ先URL（例：`https://love-counter-theta.vercel.app/`）を使う

2. **ボットリンク機能は必須**
   - これがないとログイン画面が毎回表示される
   - ユーザー体験が悪く、離脱率が上がる
   - LINEログインチャネルの「リンクされたボット」で公式アカウントを選択
...
5. **LINEログインチャネルは公開ステータスにする必要がある**
   - 開発中ステータスでは、開発者以外はログインできない
   - 一般ユーザー（友達）がアクセスするには「Published」に変更が必須
   - 公開前にプライバシーポリシーURLなどの設定が必要な場合がある

---

## 次のステップ

フェーズ4の詳細な計画は [フェーズ4 実装ガイド](../phase4/process.md) を参照してください。
...
> **作成日**: 2025-12-11
> **開発期間**: 2-3日
> **難易度**: 普通（エラー解決に時間がかかったが、調べれば解決できた）
...
- **LINE Developers Consoleが使いやすかった**
  - UIが分かりやすく、迷わず設定できた
  - ドキュメントも充実していた

- **LIFF実装でユーザーと繋がれる仕組みができた**
  - 固定ユーザーIDではなく、個別対応が可能に
  - 1対1のコミュニケーション基盤が完成

#### 大変だった点

時間がかかった部分（優先度順）：
1. **LINE設定** - チャネル作成、LIFF設定、ボットリンク機能
2. **エラー解決** - 400エラー、LIFF初期化エラー、ステータスエラー
3. **デバッグ** - スマホでのデバッグ、実機テスト

---

### ハマったポイント要約（チェックリスト）

次回LINE開発をする時は、以下をチェック：

#### 基本設定
- [ ] **LINEログインチャネルのステータスを「Published（公開）」に変更**
  - 開発中ステータスでは一般ユーザーがアクセスできない
- [ ] **ボットリンク機能を設定**
  - LINEログインチャネル ↔ Messaging APIチャネルを連携
  - これがないとログイン画面が毎回表示される
- [ ] **エンドポイントURLを正確に設定**
  - 実際のVercelデプロイ先URL（例：`https://love-counter-theta.vercel.app/`）
  - ブランチごとにURLが違う場合は統一する

#### デバッグ
- [ ] **スマホでは`alert()`で画面表示デバッグ**
  - ブラウザコンソールが見られない場合が多い
  - 重要な情報（userId、エラー内容）を画面に表示
- [ ] **早めに実機でテスト**
  - LIFF URLを開いて実際の動作確認
  - エラーは段階的に切り分ける（初期化→ユーザーID取得→送信）

#### よくあるエラー
- **400 Bad Request** → redirectUriを明示的に指定
- **「正常に作動しませんでした」** → エンドポイントURL設定ミス
- **「開発中ステータス」エラー** → チャネルを公開ステータスに変更
...
#### 技術面
- ✅ LINE Developers の設定手順を理解した
- ✅ LIFF実装の流れとハマりやすいポイントを把握
- ✅ スマホでのデバッグ技術（alert表示）を習得
- ✅ よくあるエラーパターンと解決法を記録

#### 開発姿勢
- **ユーザーと繋がれる開発を意識**
  - 1対1のコミュニケーションで楽しい体験を提供
  - ユーザーが使いやすいUIを追求
- **実機テストを早めに実施**
  - デバッグ環境を整えてから本格実装
