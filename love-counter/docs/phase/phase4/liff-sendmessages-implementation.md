...
#### Webhookで【my question】をチェック

**問題：**
- `liff.sendMessages()` を使うと、Webhookが発火してしまう
- 通常のメッセージとアプリからの送信を区別する必要がある

**解決策：**
- メッセージに「**【my question】**」が含まれているかチェック
- 含まれている場合 → Webアプリからの送信 → 専用メッセージで応答
- 含まれていない場合 → 通常のメッセージ/スタンプ → 通常の自動応答
  ...
  try {
    ...
    for (const event of events) {
      if (event.type === 'message' && event.message.type === 'text') {
        ...
        // 【my question】チェック: Webアプリからの送信
        if (messageText.includes('【my question】')) {
          await client.replyMessage(event.replyToken, {
            type: 'text',
            text: 'アプリの使用ありがとう！\nまた返信待ってるね〜😊'
          });
